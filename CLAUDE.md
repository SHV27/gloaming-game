...
>
> **THE RULES-BUDGET LAW (S6 — binding):** if a feature needs a paragraph to explain, it's wrong —
> **one line or it doesn't ship.** New identity/depth (Heroes) must be *passive, always-on, visible*
> flags that never add a mid-turn decision. And the game must be not only **action-legible** (what can
> I do now) but **causally legible** (why did that happen; how close are we; what decision won it) —
> every state change announces its **cause → effect** in-world, in one glance.
...
- `2026-07-02` — **S6 THE CROWNING (final release) — v3 is LOVED; elevate + certify, don't rebuild.** Diagnosis: game is *action*-legible but not *causally* legible ("I won but didn't know HOW"). Five ADDITIVE pillars (full spec `PLAN.md` v4): (1) **Causal legibility** — cause→effect **beats** (one `beats[]` ring-buffer, three views: transient banner / turn-log strip / Match-Story timeline), diegetic **Escape Checklist** (🏮/3 · 👥 · 🔥), the **Gate-opening moment** (`gate-open` flash on the 3rd deliver = the win-explainer), an **honest dark forecast** (exact tiles next round). (2) **Match Story** recap w/ **tiered named endings** (FLAWLESS DAWN / BY A BREATH / SWALLOWED / SO CLOSE) + loss-teacher + Play-Again/Change-Heroes. (3) **Heroes** — 5 passive one-line abilities (Swift/Lamplighter/Ember-Hearted/Unseen/Stubborn), `Player.hero`, character-select; spread must stay ≤±8 pts. (4) **The Hollow One** (the Nightmare, named + crowned) — act evolution, **full path telegraph** (`nightmare.path[]`), **Pitch bearer-hunting**, presence beats. (5) **Plannable future** — Omen suit (next card's tone), what-if hover, and **THE GRANDMASTER TEST**: a smart bot must beat the greedy bot by **≥15 pts** (objective proof skill>luck). New gates: `npm run uistate` (per-phase control contract — button-order bugs structurally impossible), and the **🏆 Certification pass** (hostile-QA subagent plays the real build; zero open defects = certified). Rules-budget law now binding (§0).
- `2026-07-03` — **S6 SHIPPED to prod (`gloaming-murex.vercel.app`), Council + Fresh-Eyes certified.** Balance GOTCHAS (tuned via the PAIRED-SEEDED `npm run grandmaster` — greedy & smart play identical seeds, killing setup-luck noise; 150-game unpaired runs bounced ±10 and were untunable): the gap↔difficulty are COUPLED through one knob, and **4p is structurally low-gap (redundancy forgives mistakes) — do NOT over-tighten 4p or the smart bot goes too slow and the gap flips NEGATIVE.** Levers that make skill pay: **frayed-step cost 1→2** (reading the fraying telegraph is now a real skill) + **torch 8→7** (torch mgmt decisive); `darkBiteFor` base `[2.2,3.0,3.9]`, `byTable` 2p .52/3p 1.02/4p 1.28 = the gap+band sweet spot. **The Unseen was a trap pick** ("overlooked until you carry", −13 spread) → repivoted to **"frayed tiles never burn your torch"** (spread 3.2, synergises w/ the new frayed cost, never a trap). Result: band **2p53/3p53/4p55, spread 2.4**, **Grandmaster +16@2p (positive everywhere)**, 0 softlocks, uistate 1272 turns. Certification fixes: **log()/beat() de-dup** consecutive identical lines (the "AI slop" Chronicle tell), Escape-Checklist labels, hero-silhouette consistency, bolder Dark forecast. Deploy: `vercel --prod --yes` on the **linked** `.vercel/project.json` (stored login present as `shauryaverma2705-3535`; the old `--scope shv-s-projects` note is stale — the link is authoritative). `everWisped→FLAWLESS` kept as a rare earned tier (a relit run wins BY A BREATH).
